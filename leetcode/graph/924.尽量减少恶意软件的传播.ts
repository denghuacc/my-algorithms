/*
 * @lc app=leetcode.cn id=924 lang=typescript
 *
 * [924] 尽量减少恶意软件的传播
 *
...
function minMalwareSpread(graph: number[][], initial: number[]): number {
  ...
  const ids: number[] = new Array(n).fill(0);
  const idToSize: Map<number, number> = new Map();
  let id = 0;
  for (let i = 0; i < n; ++i) {
    if (!ids[i]) {
      id++;
  ...
  const idToInitials: Map<number, number> = new Map();
  ...
  let ansRemoved = 0;
  for (const u of initial) {
    const removed = idToInitials.get(ids[u]) === 1 ? idToSize.get(ids[u])! : 0;
    if (removed > ansRemoved || (removed === ansRemoved && u < res)) {
      res = u;
      ansRemoved = removed;
    }
  }
