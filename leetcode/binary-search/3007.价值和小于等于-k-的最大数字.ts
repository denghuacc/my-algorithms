/*
 * @lc app=leetcode.cn id=3007 lang=typescript
 *
...
function findMaximumNumber(k: number, x: number): number {
  let l = 1n;
  let r = (BigInt(k) + 1n) << BigInt(x);
  while (l < r) {
    let m = (l + r + 1n) / 2n;
    if (getAccumulatedPrice(m) > k) {
      r = m - 1n;
    } else {
      l = m;
    }
  ...
  function getAccumulatedPrice(num: bigint) {
    let res = 0n;
    const length = 64 - Math.clz32(Number(num >> 32n));
    for (let i = x; i <= length; i += x) {
      res += getAccumulatedBitPrice(i, num);
    }
    return res;
  }

  function getAccumulatedBitPrice(x: number, num: bigint) {
