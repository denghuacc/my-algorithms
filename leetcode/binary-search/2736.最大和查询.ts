/*
 * @lc app=leetcode.cn id=2736 lang=typescript
 *
 * [2736] 最大和查询
 *
...
function maximumSumQueries(
  ...
  let j = 0;
  for (const [i, x, y] of sortedQueries) {
  ...
  function binarySearch(arr: [number, number][], target: number): number {
    let low = 0;
    let high = arr.length;
    while (low < high) {
