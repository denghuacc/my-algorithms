...
// @lc code=start
// bucket sort
var sortString = function (s: string): string {
  ...
  while (ret.length < s.length) {
    // from small to large at one traverse
    for (let i = 0; i < 26; i++) {
      if (nums[i]) {
    ...
    // from large to small at one traverse
    for (let i = 25; i >= 0; i--) {
      if (nums[i]) {
        const char = getChar(i);
        ret += char;
  ...
  function getChar(point: number): string {
    return String.fromCharCode(point + "a".charCodeAt(0));
  }
};
// @lc code=end
