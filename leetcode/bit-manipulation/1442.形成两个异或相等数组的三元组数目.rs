/*
 * @lc app=leetcode.cn id=1442 lang=rust
 *
    ...
    pub fn count_triplets(arr: Vec<i32>) -> i32 {
        ...
        for (i, &x) in arr.iter().enumerate() {
            ...
            if cnt.contains_key(&t) {
                res += cnt.get(&t).unwrap() * i - total.get(&t).unwrap();
            }
            cnt.insert(s, cnt.get(&s).unwrap_or(&0) + 1);
            total.insert(s, total.get(&s).unwrap_or(&0) + i);
            s = t;
        }
