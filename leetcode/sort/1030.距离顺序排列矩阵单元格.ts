/*
 * @lc app=leetcode.cn id=1030 lang=typescript
 *
 * [1030] 距离顺序排列矩阵单元格
 *
 * https://leetcode-cn.com/problems/matrix-cells-in-distance-order/description/
 *
 * algorithms
 * Easy (64.97%)
 * Likes:    55
 * Dislikes: 0
 * Total Accepted:    13.8K
 * Total Submissions: 20.3K
 * Testcase Example:  '1\n2\n0\n0'
 *
 * 给出 R 行 C 列的矩阵，其中的单元格的整数坐标为 (r, c)，满足 0 <= r < R 且 0 <= c < C。
 *
 * 另外，我们在该矩阵中给出了一个坐标为 (r0, c0) 的单元格。
 *
 * 返回矩阵中的所有单元格的坐标，并按到 (r0, c0) 的距离从最小到最大的顺序排，其中，两单元格(r1, c1) 和 (r2, c2)
 * 之间的距离是曼哈顿距离，|r1 - r2| + |c1 - c2|。（你可以按任何满足此条件的顺序返回答案。）
 *
 *
 *
 * 示例 1：
 *
 * 输入：R = 1, C = 2, r0 = 0, c0 = 0
 * 输出：[[0,0],[0,1]]
 * 解释：从 (r0, c0) 到其他单元格的距离为：[0,1]
 *
 *
 * 示例 2：
 *
 * 输入：R = 2, C = 2, r0 = 0, c0 = 1
 * 输出：[[0,1],[0,0],[1,1],[1,0]]
 * 解释：从 (r0, c0) 到其他单元格的距离为：[0,1,1,2]
 * [[0,1],[1,1],[0,0],[1,0]] 也会被视作正确答案。
 *
 *
 * 示例 3：
 *
 * 输入：R = 2, C = 3, r0 = 1, c0 = 2
 * 输出：[[1,2],[0,2],[1,1],[0,1],[1,0],[0,0]]
 * 解释：从 (r0, c0) 到其他单元格的距离为：[0,1,1,2,2,3]
 * 其他满足题目要求的答案也会被视为正确，例如 [[1,2],[1,1],[0,2],[1,0],[0,1],[0,0]]。
 *
 *
 *
 *
 * 提示：
 *
 *
 * 1 <= R <= 100
 * 1 <= C <= 100
 * 0 <= r0 < R
 * 0 <= c0 < C
 *
 *
 */

// @lc code=start
// sort
var allCellsDistOrder = function (
  R: number,
  C: number,
  r0: number,
  c0: number
): number[][] {
  const ret: number[][] = Array.from(new Array(R * C), () => []);
  for (let i = 0; i < R; i++) {
    for (let j = 0; j < C; j++) {
      ret[i * C + j] = [i, j];
    }
  }

  ret.sort((a, b) => {
    const x = Math.abs(a[0] - r0) + Math.abs(a[1] - c0);
    const y = Math.abs(b[0] - r0) + Math.abs(b[1] - c0);

    return x - y;
  });

  return ret;
};

// bucket sort
var allCellsDistOrder = function (
  R: number,
  C: number,
  r0: number,
  c0: number
): number[][] {
  const maxDist = Math.max(r0, R - 1 - r0) + Math.max(c0, C - 1 - c0);
  const bucket: Map<number, [number, number][]> = new Map();

  for (let i = 0; i < R; i++) {
    for (let j = 0; j < C; j++) {
      const d = dist(i, j, r0, c0);
      bucket.set(d, [...(bucket.get(d) ?? []), [i, j]]);
    }
  }

  const ret: number[][] = Array.from(new Array(R * C), () => []);
  let index = 0;
  for (let i = 0; i <= maxDist; i++) {
    for (const item of bucket.get(i)!) {
      ret[index++] = item;
    }
  }

  return ret;

  // Manhattan Distance
  function dist(r1: number, c1: number, r2: number, c2: number): number {
    return Math.abs(r1 - r2) + Math.abs(c1 - c2);
  }
};

// geometry
var allCellsDistOrder = function (
  R: number,
  C: number,
  r0: number,
  c0: number
): number[][] {
  const dr = [1, 1, -1, -1];
  const dc = [1, -1, -1, 1];
  const maxDist = Math.max(r0, R - 1 - r0) + Math.max(c0, C - 1 - c0);
  const ret: number[][] = Array.from(new Array(R * C), () => []);
  let row = r0;
  let col = c0;
  let index = 0;

  ret[index++] = [row, col];

  for (let dist = 1; dist <= maxDist; dist++) {
    row--;
    for (let i = 0; i < dr.length; i++) {
      while ((i % 2 === 0 && row !== r0) || (i % 2 !== 0 && col !== c0)) {
        if (row >= 0 && row < R && col >= 0 && col < C) {
          ret[index++] = [row, col];
        }
        row += dr[i];
        col += dc[i];
      }
    }
  }

  return ret;
};
// @lc code=end
