...
// 不知道那个字符会得到最大值，而字符是固定的两种比较少，每一种都计算出来再取最大值。
// 如果有多个字符呢？按照上面的解封，时间复杂度 0(n) * 字符种类数量；其他解法。

// 窗口移动分析：
// 右窗口每次都正常向右移动扩大窗口，增加累积的值。
// 左窗口根据实际累积的值来向右移动缩小窗口，界限条件是 > k；
// 然后在每次满足条件后，维护一下窗口的最大长度。
