...
func maxConsecutiveAnswers(answerKey string, k int) int {
	var maxConsecutiveChar func(answerKey string, k int, char byte) int
	maxConsecutiveChar = func(answerKey string, k int, char byte) (res int) {
		n := len(answerKey)
		for left, right, sum := 0, 0, 0; right < n; right++ {
	...
	res := max(maxConsecutiveChar(answerKey, k, 'T'), maxConsecutiveChar(answerKey, k, 'F'))
	return res
}
