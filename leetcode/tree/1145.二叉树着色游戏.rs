...
impl Solution {
    pub fn btree_game_winning_move(root: Option<Rc<RefCell<TreeNode>>>, n: i32, x: i32) -> bool {
        let mut arr = [0; 3];
        dfs(root, &mut arr, x, 0);
        let sum: i32 = arr.iter().sum();
        arr.iter().fold(false, |arr, &i| arr || i > sum - i)
    }
}

fn dfs(root: Option<Rc<RefCell<TreeNode>>>, arr: &mut [i32], x: i32, idx: usize) {
    if let Some(r) = root {
        if r.borrow().val == x {
            dfs(r.borrow_mut().left.take(), arr, x, 1);
            dfs(r.borrow_mut().right.take(), arr, x, 2);
        } else {
            arr[idx] += 1;
            dfs(r.borrow_mut().left.take(), arr, x, idx);
            dfs(r.borrow_mut().right.take(), arr, x, idx);
        }
    }
}
// @lc code=end
