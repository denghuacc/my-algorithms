...
impl FindElements {
    fn new(root: Option<Rc<RefCell<TreeNode>>>) -> Self {
        ...
        fn dfs(items: &mut HashSet<i32>, root: Option<Rc<RefCell<TreeNode>>>, val: i32) {
            if let Some(root) = root {
                items.insert(val);
                dfs(items, root.borrow().left.clone(), 2 * val + 1);
                dfs(items, root.borrow().right.clone(), 2 * val + 2);
            }
        }
        dfs(&mut items, root, 0);
        Self { items }
    }
