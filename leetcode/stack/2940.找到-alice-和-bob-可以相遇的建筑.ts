/*
 * @lc app=leetcode.cn id=2940 lang=typescript
 *
 ...
 * bi 。
 *
 * 请你能返回一个数组 res ，其中 res[i] 是第 i 个查询中，Alice 和 Bob 可以相遇的 最左边的建筑 。如果对于查询 i ，Alice
 * 和 Bob 不能相遇，令 res[i] 为 -1 。
 *
 *
 *
 * 示例 1：
 ...
 * 对于 res[i] != -1 ，res[i] 是 Alice 和 Bob 可以相遇的建筑中最左边建筑的下标。
 * 对于 res[i] == -1 ，不存在 Alice 和 Bob 可以相遇的建筑。
 *
 *
 * 示例 2：
 *
...
function leftmostBuildingQueries(
  ...
  const query: [number, number][][] = Array.from(new Array(n), () => []);
