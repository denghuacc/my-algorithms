...
func exclusiveTime(n int, logs []string) []int {
	...
	for _, log := range logs {
		idx, t, timestamp := split(log)

		if t == "start" {
			if len(stack) > 0 {
				res[stack[len(stack)-1][0]] += timestamp - stack[len(stack)-1][1]
				stack[len(stack)-1][1] = timestamp
			}
			stack = append(stack, []int{idx, timestamp})
		} else {
			t := stack[len(stack)-1]
			stack = stack[:len(stack)-1]
			res[t[0]] += timestamp - t[1] + 1
			if len(stack) > 0 {
				stack[len(stack)-1][1] = timestamp + 1
			}
		}
	}
	return res
...
func split(log string) (int, string, int) {
	logArr := strings.Split(log, ":")
	idxStr, t, timestampStr := logArr[0], logArr[1], logArr[2]
	var idx, timestamp int
	if i, err := strconv.Atoi(idxStr); err == nil {
		idx = i
	}
	if i, err := strconv.Atoi(timestampStr); err == nil {
		timestamp = i
	}
	return idx, t, timestamp
}
