...
function exclusiveTime(n: number, logs: string[]): number[] {
  ...
  for (const log of logs) {
    const [idx, type, timestamp] = split(log);
    if (type === "start") {
      if (stack.length) {
        res[stack.at(-1)![0]] += timestamp - stack.at(-1)![1];
        stack.at(-1)![1] = timestamp;
      }
  ...
  function split(log: string): [number, string, number] {
    const [idx, type, timestamp] = log.split(":");
    return [Number(idx), type, Number(timestamp)];
  }
}
// @lc code=end
