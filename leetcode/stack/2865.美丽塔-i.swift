/*
 * @lc app=leetcode.cn id=2865 lang=swift
 *
 * [2865] 美丽塔 I
 *
...
class Solution {
  func maximumSumOfHeights(_ maxHeights: [Int]) -> Int {
    ...
    var stack2 = [Int]()
    for i in 0..<n {
      while stack1.count > 0 && maxHeights[i] < maxHeights[stack1.last!] {
        stack1.removeLast()
      }
      if stack1.count == 0 {
        prefix[i] = (i + 1) * maxHeights[i]
      } else {
        let last = stack1.last!
    ...
    for i in (0..<n).reversed() {
      while stack2.count > 0 && maxHeights[i] < maxHeights[stack2.last!] {
        stack2.removeLast()
      }
      if stack2.count == 0 {
        suffix[i] = (n - i) * maxHeights[i]
      } else {
        let last = stack2.last!
