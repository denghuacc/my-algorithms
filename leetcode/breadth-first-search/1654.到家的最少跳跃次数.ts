/*
 * @lc app=leetcode.cn id=1654 lang=typescript
 *
 * [1654] 到家的最少跳跃次数
 *
...
function minimumJumps(
  ...
  const queue: [number, number, number][] = [[0, 1, 0]];
  const visited = new Set([0]);
  const forbiddenSet = new Set(forbidden);
  while (queue.length > 0) {
    const [position, direction, step] = queue.shift()!;
    if (position == x) {
      return step;
    }
    ...
    if (
      ...
      queue.push([nextPosition, nextDirection, step + 1]);
    }
    if (direction == 1) {
