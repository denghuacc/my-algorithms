/*
 * @lc app=leetcode.cn id=2846 lang=typescript
 *
 * [2846] 边权重均等查询
 *
...
function minOperationsQueries(
  ...
  const W = 26;
  const neighbors: Map<number, number>[] = Array.from(
    new Array(n),
    () => new Map()
  );
  ...
  const queryArr: [number, number][][] = Array.from(new Array(n), () => []);
  for (let i = 0; i < m; i++) {
    queryArr[queries[i][0]].push([queries[i][1], i]);
    queryArr[queries[i][1]].push([queries[i][0], i]);
  }

  const count: number[][] = Array.from(new Array(n), () =>
  ...
  function tarjan(node: number, parent: number) {
    if (parent !== -1) {
  ...
  function find(uf: number[], i: number) {
    if (uf[i] === i) {
      return i;
    }
    uf[i] = find(uf, uf[i]);
