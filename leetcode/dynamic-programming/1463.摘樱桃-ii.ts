/*
 * @lc app=leetcode.cn id=1463 lang=typescript
 *
 * [1463] 摘樱桃 II
 *
...
function cherryPickup(grid: number[][]): number {
  ...
  let f: number[][] = Array.from(new Array(n), () => Array(n).fill(-1));
  let g: number[][] = Array.from(new Array(n), () => Array(n).fill(-1));
  ...
  let res = 0;
  for (let j1 = 0; j1 < n; ++j1) {
    res = Math.max(res, Math.max(...f[j1]));
  }
  return res;
}
// @lc code=end
