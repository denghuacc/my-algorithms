...
// @lc code=start
// dp
var videoStitching = function (clips: number[][], T: number): number {
  const dp: number[] = new Array(T + 1).fill(Number.MAX_SAFE_INTEGER - 1);
  dp[0] = 0;
  for (let i = 1; i <= T; i++) {
...
// greedy
var videoStitching = function (clips: number[][], T: number): number {
  const maxN: number[] = new Array(T).fill(0);
  let ret = 0;
  let last = 0;
  let pre = 0;
