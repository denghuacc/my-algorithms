...
function minimumTime(n: number, relations: number[][], time: number[]): number {
  const prev: number[][] = Array.from(new Array(n + 1), () => []);
  for (const [v, w] of relations) {
    prev[w].push(v);
  }
  let res = 0;
  ...
  function dp(i: number, time: number[]): number {
    if (!memo[i]) {
      let cur = 0;
      for (const p of prev[i]) {
        cur = Math.max(cur, dp(p, time));
      }
      cur += time[i - 1];
      memo[i] = cur;
    }
