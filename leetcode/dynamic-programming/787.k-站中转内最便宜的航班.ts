...
function findCheapestPrice(
  ...
  // dp[t][i] -> 通过恰好 t 次航班，从出发城市 src 到达城市 i 需要的最小花费
  const dp = Array.from(new Array(k + 2), () => new Array(n).fill(INF));
  dp[0][src] = 0;
  for (let t = 1; t <= k + 1; t++) {
    for (const [j, i, cost] of flights) {
      dp[t][i] = Math.min(dp[t][i], dp[t - 1][j] + cost);
    }
  }
  let ret = INF;
  for (let t = 1; t <= k + 1; t++) {
    ret = Math.min(ret, dp[t][dst]);
  }
