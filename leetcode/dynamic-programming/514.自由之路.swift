...
class Solution {
  func findRotateSteps(_ ring: String, _ key: String) -> Int {
    ...
    var pos = [[Int]](repeating: [Int](), count: 26)
    for i in 0..<n {
      pos[getCode(ring, i)].append(i)
    }

    var dp = [[Int]](repeating: [Int](repeating: Int.max, count: n), count: m)
    for i in pos[getCode(key, 0)] {
      dp[0][i] = min(i, n - i) + 1
    }
    for i in 1..<m {
      for j in pos[getCode(key, i)] {
        for k in pos[getCode(key, i - 1)] {
  ...
  func getCode(_ ring: String, _ key: Int) -> Int {
    return Int(ring[ring.index(ring.startIndex, offsetBy: key)].asciiValue!) - 97
  }
}
// @lc code=end
