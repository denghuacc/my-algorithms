/*
 * @lc app=leetcode.cn id=2383 lang=rust
 *
 * [2383] 赢得比赛需要的最少训练时长
 *
...
impl Solution {
    pub fn min_number_of_hours(
        ...
        let mut res = 0;
        let total_energy = energy.iter().sum::<i32>();
        if initial_energy <= total_energy {
            res += total_energy - initial_energy + 1;
        }
        let mut initial_experience = initial_experience;
        for e in experience {
            if initial_experience <= e {
                res += e - initial_experience + 1;
                initial_experience = 2 * e + 1;
            } else {
