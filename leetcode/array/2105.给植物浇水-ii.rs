...
impl Solution {
    pub fn minimum_refill(plants: Vec<i32>, capacity_a: i32, capacity_b: i32) -> i32 {
        let n = plants.len();
        let mut cap_a = capacity_a;
        let mut cap_b = capacity_b;
        let mut res = 0;
        let mut i = 0;
        let mut j = n - 1;
        while i <= j {
            let plant_a = plants[i];
            let plant_b = plants[j];
            if i == j {
                if cap_a < plant_a && cap_b < plant_b {
                    res += 1;
                }
                break;
            }
            if cap_a >= plant_a {
                ...
                cap_a = capacity_a - plant_a;
                res += 1;
            }
            if cap_b >= plant_b {
                cap_b -= plant_b;
            } else {
                cap_b = capacity_b - plant_b;
                res += 1;
            }
            i += 1;
            j -= 1;
