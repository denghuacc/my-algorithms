...
impl Solution {
    pub fn min_operations_max_profit(
        ...
        if boarding_cost * 4 - running_cost < 0 {
            return -1;
        }
        ...
        let mut remain = 0;
        let mut boarding_time = 0;
        let mut running_time = 0;

        for customer in customers {
            remain += customer;
            let count = if remain > 4 { 4 } else { remain };
            if count > 0 {
                boarding_time += count;
                remain -= count;
            }
            running_time += 1;
            let profit = boarding_cost * boarding_time - running_cost * running_time;
