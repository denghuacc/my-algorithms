...
impl Solution {
    pub fn escape_ghosts(ghosts: Vec<Vec<i32>>, target: Vec<i32>) -> bool {
        ...
        let mut min = (target[0] - ghosts[0][0]).abs() + (target[1] - ghosts[0][1]).abs();
        for i in 1..ghosts.len() {
            let dis = (target[0] - ghosts[i][0]).abs() + (target[1] - ghosts[i][1]).abs();
            if dis < min {
                min = dis;
            }
        }
        min > (target[0].abs() + target[1].abs())
    }
}
// @lc code=end
