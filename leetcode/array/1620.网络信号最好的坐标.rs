 ...
 *
 * 数组  towers  中包含一些网络信号塔，其中 towers[x] = [xi, yi, qi] 表示第 i 个网络信号塔的坐标是 (xi, yi)
...
impl Solution {
    pub fn best_coordinate(towers: Vec<Vec<i32>>, radius: i32) -> Vec<i32> {
        ...
        for tower in towers.iter() {
            x_max = x_max.max(tower[0]);
            y_max = y_max.max(tower[1]);
        }
        ...
        for x in 0..=x_max {
            for y in 0..=y_max {
                let mut quality = 0;
                for tower in towers.iter() {
                    let dist = (tower[0] - x) * (tower[0] - x) + (tower[1] - y) * (tower[1] - y);
                    if dist <= radius * radius {
                        let sqrt_dist = f64::sqrt(dist as f64);
                        quality += (tower[2] as f64 / (1.0 + sqrt_dist)) as i32;
                    }
                }
