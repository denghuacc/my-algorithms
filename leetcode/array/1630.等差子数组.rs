...
impl Solution {
    pub fn check_arithmetic_subarrays(nums: Vec<i32>, l: Vec<i32>, r: Vec<i32>) -> Vec<bool> {
        ...
        for i in 0..n {
            ...
            let mut min_num = nums[left];
            let mut max_num = nums[left];
            for j in left + 1..=right {
                min_num = min_num.min(nums[j]);
                max_num = max_num.max(nums[j]);
            }
            if min_num == max_num {
                res[i] = true;
                continue;
            }
            if (max_num - min_num) % (right - left) as i32 != 0 {
                res[i] = false;
                continue;
            }
            let d = (max_num - min_num) / (right - left) as i32;
            let mut flag = true;
            let mut seem = vec![false; right - left + 1];
            for j in left..=right {
                if (nums[j] - min_num) % d != 0 {
                    flag = false;
                    break;
                }
                let t = ((nums[j] - min_num) / d) as usize;
                if seem[t] {
                    flag = false;
                    break;
                }
