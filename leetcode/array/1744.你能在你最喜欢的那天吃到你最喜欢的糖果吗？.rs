/*
 * @lc app=leetcode.cn id=1744 lang=rust
 *
...
impl Solution {
    ...
    //     let mut sum = vec![0 as i64; n + 1]; // i32 is too small
    //     for i in 1..n + 1 {
    //         sum[i] = sum[i - 1] + candies_count[i - 1] as i64;
    //     }
    //     let q = queries.len();
    //     let mut ret = Vec::with_capacity(q);
    //     for query in &queries {
    //         let days = query[1] as i64 + 1;
    //         if days > sum[query[0] as usize + 1] {
    //             ret.push(false)
    //         } else if days * query[2] as i64 <= sum[query[0] as usize] {
    //             ret.push(false)
    //         } else {
    //             ret.push(true)
    //         }
    ...
    pub fn can_eat(candies_count: Vec<i32>, queries: Vec<Vec<i32>>) -> Vec<bool> {
        let sum = candies_count
            .iter()
            .enumerate()
            .fold(Vec::new(), |mut s, (idx, &c)| {
                if idx == 0 {
                    s.push(0);
                    s.push(c as i64);
                } else {
                    s.push(c as i64 + s[s.len() - 1]);
                }
                return s;
            });
        ...
        queries
            ...
            .map(|q| {
                ...
                let d = q[1] as i64 + 1;
                let cap = q[2] as i64;
                if (d - 1) >= sum[t as usize] {
                    return false;
                } else if cap * d <= sum[t as usize - 1] {
                    return false;
                }
                return true;
            })
