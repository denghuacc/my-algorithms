...
// @lc code=start
var calculateTax = function (brackets: number[][], income: number): number {
  if (income === 0) {
    return 0;
  }
  let res = 0;
  ...
  for (const [upper, percent] of brackets) {
    if (income > upper) {
      res += ((upper - preUpper) * percent) / 100;
      preUpper = upper;
    } else {
      const diff = income - preUpper;
...
var calculateTax = function (brackets: number[][], income: number): number {
  ...
  let lower = 0;
  for (const [upper, percent] of brackets) {
    res += (Math.min(upper, income) - lower) * percent;
    if (income <= upper) {
      break;
    }
    lower = upper;
