 ...
 *
 * 现在，返回一个整数数组 reswer 作为答案，其中 reswer[x] = y 的前提是，在所有拥有的钱肯定不少于 person x
...
impl Solution {
    pub fn loud_and_rich(richer: Vec<Vec<i32>>, quiet: Vec<i32>) -> Vec<i32> {
        ...
        let mut res = vec![-1; n];
        for i in 0..n {
            Self::dfs(i, &graph, &quiet, &mut res);
        }
        res
    }

    fn dfs(i: usize, graph: &Vec<Vec<usize>>, quiet: &Vec<i32>, res: &mut Vec<i32>) {
        if res[i] != -1 {
            return;
        }
        res[i] = i as i32;
        for &j in &graph[i] {
            Self::dfs(j, graph, quiet, res);
            if quiet[res[j] as usize] < quiet[res[i] as usize] {
                res[i] = res[j];
            }
        }
    }
