...
function canIWin(maxChoosableInteger: number, desiredTotal: number): boolean {
  ...
  function dfs(userNumbers: number, currentTotal: number): boolean {
    if (!memo.has(userNumbers)) {
      let res = false;
      for (let i = 0; i < maxChoosableInteger; i++) {
        if (((userNumbers >> i) & 1) === 0) {
          ...
          if (!dfs(userNumbers | (1 << i), currentTotal + i + 1)) {
            res = true;
            break;
          }
        }
      }
      memo.set(userNumbers, res);
    }
    return memo.get(userNumbers)!;
  }
}
// @lc code=end
