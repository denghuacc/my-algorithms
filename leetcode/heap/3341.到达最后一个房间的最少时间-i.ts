/*
 * @lc app=leetcode.cn id=3341 lang=typescript
 *
 ...
 *
 *
 * 2 <= m == moveTime.length <= 50
 * 2 <= n == moveTime[i].length <= 50
 * 0 <= moveTime[i][j] <= 10^9
 *
 *
 */
...
function minTimeToReach(moveTime: number[][]): number {
  ...
  while (!q.isEmpty()) {
    ...
    for (const [dx, dy] of DIRS) {
      ...
      const nt = Math.max(d[x][y], moveTime[nx][ny]) + 1;
      if (nt < d[nx][ny]) {
        d[nx][ny] = nt;
        q.push([nx, ny, nt]);
      }
