...
function findCrossingTime(n: number, k: number, time: number[][]): number {
  ...
  const waitLeft = new Heap<number>((x, y) => {
    const timeX = time[x][0] + time[x][2];
    const timeY = time[y][0] + time[y][2];
    return timeX !== timeY ? timeX > timeY : x > y;
  });
  const waitRight = new Heap<number>((x, y) => {
    const timeX = time[x][0] + time[x][2];
    const timeY = time[y][0] + time[y][2];
    return timeX !== timeY ? timeX > timeY : x > y;
  });
  const workLeft = new Heap<number[]>((x, y) => {
    if (x[0] !== y[0]) {
      return x[0] < y[0];
    } else {
      return x[1] < y[1];
  ...
  const workRight = new Heap<number[]>((x, y) => {
    if (x[0] !== y[0]) {
      return x[0] < y[0];
    } else {
      return x[1] < y[1];
  ...
  while (remain > 0 || !workRight.isEmpty() || !waitRight.isEmpty()) {
    ...
    while (!workLeft.isEmpty() && workLeft.peek()![0] <= curTime) {
      waitLeft.push(workLeft.pop()![1]);
    }
    while (!workRight.isEmpty() && workRight.peek()![0] <= curTime) {
      waitRight.push(workRight.pop()![1]);
    }
    if (!waitRight.isEmpty()) {
      // 2. 若右侧有工人在等待，则取出优先级最低的工人并过桥
      const id = waitRight.pop()!;
      curTime += time[id][2];
      workLeft.push([curTime + time[id][3], id]);
    } else if (remain > 0 && !waitLeft.isEmpty()) {
      // 3. 若右侧还有箱子，并且左侧有工人在等待，则取出优先级最低的工人并过桥
      const id = waitLeft.pop()!;
      curTime += time[id][0];
      workRight.push([curTime + time[id][1], id]);
      remain--;
    } else {
