...
func countDaysTogether(arriveAlice string, leaveAlice string, arriveBob string, leaveBob string) int {
	...
	preSum := make([]int, len(days)+1)
	for i, day := range days {
		preSum[i+1] = preSum[i] + day
	}
	arriveA, leaveA := getUniDate(arriveAlice, preSum), getUniDate(leaveAlice, preSum)
	arriveB, leaveB := getUniDate(arriveBob, preSum), getUniDate(leaveBob, preSum)

	if leaveA < arriveB || leaveB < arriveA {
		return 0
	}
	return min(leaveA, leaveB) - max(arriveA, arriveB) + 1
}

func getUniDate(dateStr string, preSum []int) int {
	dates := strings.Split(dateStr, "-")
	month, _ := strconv.Atoi(dates[0])
	day, _ := strconv.Atoi(dates[1])
