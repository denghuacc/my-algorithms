/*
 * @lc app=leetcode.cn id=2288 lang=typescript
 *
 * [2288] 价格减免
 *
...
function discountPrices(sentence: string, discount: number): string {
  ...
  const n = res.length;
  for (let i = 0; i < n; i++) {
    if (res[i].at(0) === "$" && res[i].length > 1) {
      const price = res[i].slice(1);
      if (!isDigit(price)) {
        continue;
      }
      const discounted = (Number(price) * (100 - discount)) / 100;
      if (discounted >= 1e10) {
        continue;
      }
      res[i] = `$${discounted.toFixed(2)}`;
    }
  }
  return res.join(" ");
