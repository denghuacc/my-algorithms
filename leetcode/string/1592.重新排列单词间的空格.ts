...
function reorderSpaces(text: string): string {
  const [spaceCount, words] = getSpaceCountAndWords(text);
  const wordCount = words.length;
  const gap = wordCount === 1 ? 0 : Math.floor(spaceCount / (wordCount - 1));
  const tail = wordCount === 1 ? spaceCount : spaceCount % (wordCount - 1);
  const gapSpace = " ".repeat(gap);
  const tailSpace = " ".repeat(tail);

  let res = "";
  for (let i = 0; i < wordCount; i++) {
    const word = words[i];
    res += word;
    res += i !== wordCount - 1 ? gapSpace : tailSpace;
  }
  return res;

  function getSpaceCountAndWords(text: string): [number, string[]] {
    let spaceCount = 0;
    const words: string[] = [];
