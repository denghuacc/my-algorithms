/*
 * @lc app=leetcode.cn id=1953 lang=rust
 *
 * [1953] 你可以工作的最大周数
 *
...
impl Solution {
    pub fn number_of_weeks(milestones: Vec<i32>) -> i64 {
        let milestones: Vec<i64> = milestones.into_iter().map(|x| x as i64).collect();
        let longest = *milestones.iter().max().unwrap();
        let sum = milestones.iter().sum::<i64>();
        let rest = sum - longest;
        if longest > rest + 1 {
            2 * rest + 1
        } else {
            sum
