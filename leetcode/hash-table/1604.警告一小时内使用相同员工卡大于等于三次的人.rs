/*
* @lc app=leetcode.cn id=1604 lang=rust
*
...
impl Solution {
    pub fn alert_names(key_name: Vec<String>, key_time: Vec<String>) -> Vec<String> {
        ...
        let mut name_to_times = HashMap::new();
        let n = key_name.len();
        for i in 0..n {
            let name = key_name[i].clone();
            let times = name_to_times.entry(name).or_insert(vec![]);
            times.push(key_time[i].clone());
        }
        let mut res: Vec<String> = vec![];
        let name_to_times = name_to_times.into_iter();
        for (name, times) in name_to_times {
            if Solution::enable_alert(times) {
                res.push(name);
            }
        }
        res.sort();
    ...
    fn enable_alert(times: Vec<String>) -> bool {
        if times.len() < 3 {
            return false;
        }
        let mut times: Vec<i32> = times
            .iter()
            .map(|time| {
                let str_arr = time.split(':').collect::<Vec<&str>>();
                return str_arr[0].parse::<i32>().unwrap() * 60
                    + str_arr[1].parse::<i32>().unwrap();
            })
            .collect();
        times.sort();
        let n = times.len();
