...
function arrayRankTransform(arr: number[]): number[] {
  ...
  const numToSortedIdx: Map<number, number> = new Map();
  for (const num of sortedArr) {
    if (!numToSortedIdx.has(num))
      numToSortedIdx.set(num, numToSortedIdx.size + 1);
  }
  const res: number[] = [];
  for (const num of arr) {
    const sortedIdx = numToSortedIdx.get(num)!;
    res.push(sortedIdx);
  }
  return res;
}
