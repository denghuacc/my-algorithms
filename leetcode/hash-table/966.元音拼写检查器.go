...
func spellchecker(wordlist []string, queries []string) []string {
	...
	for _, word := range wordlist {
		exactWords[word] = struct{}{}
		lower := strings.ToLower(word)
		if _, ok := caseInsensitive[lower]; !ok {
			caseInsensitive[lower] = word
		}
		if _, ok := vowelError[replaceVowels(lower)]; !ok {
			vowelError[replaceVowels(lower)] = word
		}
	}

	result := make([]string, len(queries))
	for i, query := range queries {
		if _, ok := exactWords[query]; ok {
			result[i] = query
			continue
		}
		if word, ok := caseInsensitive[strings.ToLower(query)]; ok {
			result[i] = word
			continue
		}
		if word, ok := vowelError[replaceVowels(strings.ToLower(query))]; ok {
			result[i] = word
			continue
		}
		result[i] = ""
