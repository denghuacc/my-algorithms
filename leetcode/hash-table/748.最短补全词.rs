...
impl Solution {
    pub fn shortest_completing_word(license_plate: String, words: Vec<String>) -> String {
        let get_cnt = |word: &String, is_word: bool| {
            let mut cnt = [0; 26];
            let word = word.to_lowercase();
            for c in word.chars() {
                if is_word || c.is_alphabetic() {
        ...
        let lp_cnt = get_cnt(&license_plate, false);
        let mut idx = usize::MAX;
        for (k, word) in words.iter().enumerate() {
            let word_cnt = get_cnt(word, true);
            let mut ok = true;
            for i in 0..26 {
                if lp_cnt[i] > word_cnt[i] {
                    ok = false;
                    break;
                }
            }
            if ok && (idx == usize::MAX || word.len() < words[idx].len()) {
                idx = k;
            }
        }
        words[idx].clone()
