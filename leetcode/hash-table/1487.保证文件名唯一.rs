/*
 * @lc app=leetcode.cn id=1487 lang=rust
 *
 * [1487] 保证文件名唯一
 *
    ...
    pub fn get_folder_names(names: Vec<String>) -> Vec<String> {
        ...
        for name in names {
            if !map.contains_key(&name) {
                ...
                let mut k = *map.get(&name).unwrap();
                while map.contains_key(&addSuffix(&name, k)) {
                    k += 1;
                }
                let new_name = addSuffix(&name, k);
                res.push(new_name.clone());
                map.insert(new_name.clone(), 1);
                map.insert(name.clone(), k + 1);
            }
        }
        res
    }
...
fn addSuffix(name: &String, k: i32) -> String {
