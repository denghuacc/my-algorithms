...
function alertNames(keyName: string[], keyTime: string[]): string[] {
  ...
  const nameToTimes: Map<string, string[]> = new Map();
  for (let i = 0; i < n; i++) {
    if (!nameToTimes.has(keyName[i])) {
      nameToTimes.set(keyName[i], [keyTime[i]]);
    } else {
      const times = nameToTimes.get(keyName[i])!;
      times.push(keyTime[i]);
    }
  }
  const res = [];
  for (const [name, times] of nameToTimes.entries()) {
    if (enableAlert(times)) {
      res.push(name);
    }
  }
  res.sort();
  ...
  function enableAlert(times: string[]): boolean {
    if (times.length < 3) {
      return false;
    }
    const timesStamp = times.map((time) => {
      const strArr = time.split(":");
      return Number(strArr[0]) * 60 + Number(strArr[1]);
    });
    timesStamp.sort((a, b) => a - b);
    for (let i = 0; i < timesStamp.length; i++) {
      if (timesStamp[i + 2] - timesStamp[i] <= 60) {
        return true;
      }
    }
    return false;
