...
function shortestCompletingWord(licensePlate: string, words: string[]): string {
  const lp = licensePlate.toLowerCase().replace(/\s|\d/g, "");
  const mapLp = new Map<string, number>();
  const setLp = new Set<string>();
  for (const ch of lp) {
    setLp.add(ch);
    mapLp.set(ch, (mapLp.get(ch) ?? 0) + 1);
  }
  ...
  matchWords.sort((a, b) => a.length - b.length);
  return matchWords[0];

  function hasMatch(word: string): boolean {
    const mapWord = new Map<string, number>();
    for (const ch of word) {
      mapWord.set(ch, (mapWord.get(ch) ?? 0) + 1);
    }
    for (const ch of setLp) {
      if (mapWord.get(ch) === undefined || mapWord.get(ch)! < mapLp.get(ch)!) {
        return false;
      }
    }
    return true;
