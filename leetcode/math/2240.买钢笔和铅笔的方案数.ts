...
// @lc code=start
var waysToBuyPensPencils = function (
  total: number,
  cost1: number,
  cost2: number
): number {
  ...
  const maxCount = Math.floor(total / maxCost);
  const minCost = Math.min(cost1, cost2);
  let idx = 0;
  let res = 0;
  while (idx <= maxCount) {
    const rest = total - idx * maxCost;
    res += Math.floor(rest / minCost) + 1;
    idx++;
  }
  return res;
};
...
var waysToBuyPensPencils = function (
  ...
  while (cnt * cost1 <= total) {
    res += Math.floor((total - cnt * cost1) / cost2) + 1;
    cnt++;
  }
  return res;
};
