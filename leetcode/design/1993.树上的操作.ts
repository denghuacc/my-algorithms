/*
 * @lc app=leetcode.cn id=1993 lang=typescript
 *
 * [1993] 树上的操作
 *
...
class LockingTree {
  ...
  constructor(parent: number[]) {
    ...
    this.children = new Array(n).fill(0).map(() => []);
    for (let i = 0; i < n; i++) {
      const p = parent[i];
      if (p != -1) {
  ...
  private checkAndUnlockDescendant(num: number): number {
    let res = this.lockNodeUser[num] != -1 ? 1 : 0;
    this.lockNodeUser[num] = -1;
