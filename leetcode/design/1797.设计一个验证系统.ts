...
class AuthenticationManager {
  ...
  generate(tokenId: string, currentTime: number): void {
    this.idToTime.set(tokenId, currentTime + this.timeToLive);
  }

  renew(tokenId: string, currentTime: number): void {
    const time = this.idToTime.get(tokenId) ?? 0;
    if (time > currentTime) {
  ...
  countUnexpiredTokens(currentTime: number): number {
    let count = 0;
    for (const time of this.idToTime.values()) {
      if (time > currentTime) {
        count++;
      }
    }
