...
class SmallestInfiniteSet {
  ...
  popSmallest(): number {
    ...
    this.deleteSet.add(smallest);
    let curSmallest = smallest + 1;
    while (this.deleteSet.has(curSmallest)) {
      curSmallest++;
    }
    this.smallest = curSmallest;
    return smallest;
  }

  addBack(num: number): void {
    if (this.deleteSet.has(num)) {
      this.deleteSet.delete(num);
    }
