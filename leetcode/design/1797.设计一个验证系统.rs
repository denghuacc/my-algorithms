/*
 * @lc app=leetcode.cn id=1797 lang=rust
 *
 * [1797] 设计一个验证系统
 *
...
impl AuthenticationManager {
    ...
    fn renew(&mut self, token_id: String, current_time: i32) {
        if self.id_to_time.contains_key(&token_id) {
            let time = *self.id_to_time.get(&token_id).unwrap();
