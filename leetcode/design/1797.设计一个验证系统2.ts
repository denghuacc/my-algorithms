...
class AuthenticationManager {
  ...
  generate(tokenId: string, currentTime: number): void {
    ...
    this.idToNode.set(tokenId, node);
    // insert
  ...
  renew(tokenId: string, currentTime: number): void {
    if (
      this.idToNode.has(tokenId) &&
      this.idToNode.get(tokenId)!.expire > currentTime
    ) {
      const node = this.idToNode.get(tokenId)!;
  ...
  countUnexpiredTokens(currentTime: number): number {
    while (
      this.head.next!.expire > 0 &&
      this.head.next!.expire <= currentTime
    ) {
...
class Node {
  ...
  constructor(key: string, expire: number, prev?: Node, next?: Node) {
    this.key = key;
    this.expire = expire;
    this.prev = prev;
    this.next = next;
