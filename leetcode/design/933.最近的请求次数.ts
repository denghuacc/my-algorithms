...
class RecentCounter {
  ...
  constructor() {
    this.requests = [];
    this.prevTime = 0;
    this.prevTimeIndex = -1;
  }

  ping(t: number): number {
    let count = 1;
    this.requests.push(t);
    if (this.prevTimeIndex === -1) {
      this.prevTimeIndex = 0;
      this.prevTime = t;
      return count;
    }
    const curPrevTime = this.prevTime;
    const n = this.requests.length;
    const start = t - 3000;

    if (curPrevTime >= start) {
      count = n - this.prevTimeIndex;
    } else {
      while (this.requests[this.prevTimeIndex] < start) {
        this.prevTimeIndex++;
      }
      this.prevTime = this.requests[this.prevTimeIndex];
      count = n - this.prevTimeIndex;
    }
