...
public class Main {
  ...
  // 可以看出, Shell Sort 虽然慢于高级的排序方式, 但仍然是非常有竞争力的一种排序算法
  // 其所花费的时间完全在可以容忍的范围内, 远不像 O(n^2) 的排序算法, 在数据量较大的时候无法忍受
  // 同时, Shell Sort 实现简单, 只使用循环的方式解决排序问题, 不需要实现递归, 不占用系统占空间, 也不依赖随机数
  // 所以, 如果算法实现所使用的环境不利于实现复杂的排序算法, 或者在项目工程的测试阶段, 完全可以暂时使用 Shell Sort来进行排序任务
  public static void main(String[] args) {

    // 测试 T 个测试用例, 每个测试用例的数组大小为 n
    int T = 100;
    int N = 1000000;
    ...
    for (int i = 0; i < T; i++) {
      ...
      time2 += SortTestHelper.testSort2("MergeSort3", arr2);
      time3 += SortTestHelper.testSort2("QuickSort2", arr3);
      time4 += SortTestHelper.testSort2("QuickSort3", arr4);
      time5 += SortTestHelper.testSort2("QuickSort4", arr5);
    }
