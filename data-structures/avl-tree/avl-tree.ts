...
export default class AVLTree<K, V> {
  ...
  private isBalancedTree(root: Node<K, V>): boolean {
    if (root == null) {
      return true
    }
  ...
  // 获取节点 root 的高度
  private getHeight(root: Node<K, V>): number {
  ...
  private removeNode(root: Node<K, V>, key: K) {
    ...
    // retNode 可能为空
    if (retNode == null) return undefined

    // 删除元素后需要维护树的平衡，逻辑和添加元素一样
    // 更新 height
    retNode.height =
      1 +
      Math.max(this.getHeight(retNode.left!), this.getHeight(retNode.right!))
